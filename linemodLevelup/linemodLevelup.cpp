...
const static uchar lut0 = 0;
const static uchar lut1 = 0;
const static uchar lut2 = 0;
const static uchar lut3 = 1;
const static uchar lut4 = 4;
CV_DECL_ALIGNED(16) static const unsigned char SIMILARITY_LUT[256] =
{lut0, lut4, lut3, lut4, lut2, lut4, lut3, lut4, lut1, lut4, lut3, lut4, lut2, lut4, lut3, lut4,
 lut0, lut0, lut1, lut1, lut2, lut2, lut2, lut2, lut3, lut3, lut3, lut3, lut3, lut3, lut3, lut3,
 lut0, lut3, lut4, lut4, lut3, lut3, lut4, lut4, lut2, lut3, lut4, lut4, lut3, lut3, lut4, lut4,
 lut0, lut1, lut0, lut1, lut1, lut1, lut1, lut1, lut2, lut2, lut2, lut2, lut2, lut2, lut2, lut2,
 lut0, lut2, lut3, lut3, lut4, lut4, lut4, lut4, lut3, lut3, lut3, lut3, lut4, lut4, lut4, lut4,
 lut0, lut2, lut1, lut2, lut0, lut2, lut1, lut2, lut1, lut2, lut1, lut2, lut1, lut2, lut1, lut2,
 lut0, lut1, lut2, lut2, lut3, lut3, lut3, lut3, lut4, lut4, lut4, lut4, lut4, lut4, lut4, lut4,
 lut0, lut3, lut2, lut3, lut1, lut3, lut2, lut3, lut0, lut3, lut2, lut3, lut1, lut3, lut2, lut3,
 lut0, lut0, lut1, lut1, lut2, lut2, lut2, lut2, lut3, lut3, lut3, lut3, lut3, lut3, lut3, lut3,
 lut0, lut4, lut3, lut4, lut2, lut4, lut3, lut4, lut1, lut4, lut3, lut4, lut2, lut4, lut3, lut4,
 lut0, lut1, lut0, lut1, lut1, lut1, lut1, lut1, lut2, lut2, lut2, lut2, lut2, lut2, lut2, lut2,
 lut0, lut3, lut4, lut4, lut3, lut3, lut4, lut4, lut2, lut3, lut4, lut4, lut3, lut3, lut4, lut4,
                                   ...
                                   const std::vector<Mat> &masks_ori)
{
    ...
    auto find_matches = [&](std::vector<Ptr<QuantizedPyramid>>& quantizers, const std::vector<std::string> &class_ids){
        ...
        {
            // Match all templates
            auto it = class_templs_structure.begin(), itend = class_templs_structure.end();
            for (; it != itend; ++it)
                matchClass_by_structure(lm_pyramid, sizes, threshold, active_ratio, matches, it->first, it->second);
        }
        else
        {
            ...
            {
                auto it = class_templs_structure.find(class_ids[i]);
                if (it != class_templs_structure.end())
                    matchClass_by_structure(lm_pyramid, sizes, threshold, active_ratio, matches, it->first, it->second);
            }
        }
...
Detector::TemplateStructure Detector::build_templ_structure(Pose_structure &structure, PoseRenderer &renderer)
{
    ...
    // find cluster of similar pose and their map in each level
    ...
    // find map of cluster between two adj levels
    ...
    // transfer map to vector & corresponding tree
    ...
    for(auto& m: wanted_maps[T_at_level.size()-1]){
        ...
        for(auto p_id: m.second){
            ...
            templ_forest.back().push_back(id_level2unique(p_id, T_at_level.size()-2));
        }
...
bool Detector::is_similar(Mat &pose1, Mat &pose2, int pyr_level, int stride, PoseRenderer &renderer)
{
    ...
    }
    {
        ...
        cv::Mat mask(bbox1.height+2*padding, bbox1.width+2*padding, CV_8UC1, cv::Scalar(0));
        cv::Mat depth(bbox1.height+2*padding, bbox1.width+2*padding, CV_16UC1, cv::Scalar(0));
        dep_masks[1][0](bbox1).copyTo(depth(Rect(padding, padding, bbox1.width, bbox1.height)));
        dep_masks[1][1](bbox1).copyTo(mask(Rect(padding, padding, bbox1.width, bbox1.height)));
        dep_masks[1][1] = depth(cv::Rect(0, 0, depth.cols/16*16, depth.rows/16*16)).clone();
        dep_masks[1][0] = mask(cv::Rect(0, 0, mask.cols/16*16, mask.rows/16*16)).clone();
    }
    std::vector<Template> templs(2);
    std::vector<LinearMemories> lm_level(2, std::vector<cv::Mat>(8));
    ...
    {
        {
            ...
            templ.height = max_y - min_y;
            templ.width = max_x - min_x;
            for (int j = 0; j < (int)templ.features.size(); ++j)
            {
                templ.features[j].x -= min_x;
                templ.features[j].y -= min_y;
            }
    ...
    auto similarity_custom = [&stride, &dep_masks](Template& templ, LinearMemories& linear_memories){
        int W = dep_masks[1][1].cols / stride;
        int H = dep_masks[1][1].rows / stride;
    ...
    cv::minMaxLoc(score, &minVal, &maxVal, &min_loc, &max_loc);

    const bool view_templ = false;
    if(view_templ){
        cout << score0.at<uint16_t>(max_loc.y, max_loc.x) << endl;
        cout << score1.at<uint16_t>(max_loc.y, max_loc.x) << endl;
        cout << "max loc: " << max_loc.x*stride << " " << max_loc.y*stride << endl;
        cout << "max val: " << maxVal << endl;
        cout << "max score: " << (templs[0].features.size() + templs[1].features.size())*4 << endl;

        cv::Mat test_img = dep_masks[1][0].clone();
        cvtColor(test_img, test_img, CV_GRAY2BGR);
        {
            auto& templ = templs[0];
            for(int i=0; i<templ.features.size(); i++){
                auto feat = templ.features[i];
                cv::circle(test_img, {feat.x + max_loc.x*stride, feat.y + max_loc.y*stride}, 2, {0, 0, 255}, -1);
            }
        }
        {
            auto& templ = templs[1];
            for(int i=0; i<templ.features.size(); i++){
                auto feat = templ.features[i];
                cv::circle(test_img, {feat.x + max_loc.x*stride, feat.y + max_loc.y*stride}, 2, {0, 255, 0}, -1);
            }
        }
